...
hbb_name = 'hesabdesk' + ('.exe' if windows else '')
...
def get_features(args):
    ...
    if args.drm:
        ...
        # package NAMED `hesabdesk` carrying the consent-bypass backend and the root-side uinput
...
def generate_control_file(version):
    ...
    content = """Package: hesabdesk
Section: net
Priority: optional
Version: %s
Architecture: %s
Maintainer: hesabdesk <info@hesabdesk.com>
Homepage: https://hesabdesk.com
...
# commit — the same way hesabdesk sources its other native build deps (vcpkg,
# flutter_rust_bridge, ...), rather than carrying a git submodule. It is the ONLY
# pin for the drm backend: hesabdesk dlopens this .so at runtime and does not depend on
...
def build_libdrmtap_so():
    ...
    if not os.path.exists(os.path.join(build_dir, 'build.ninja')):
        # -Dhelper=disabled: hesabdesk never uses the privileged helper. The capture context is
        ...
        # entirely. Requested by the maintainer on hesabdesk#16242.
    ...
    # the shared one explicitly (hesabdesk dlopens the .so and never needs the archive).
...
def _assert_so_has_no_helper(so_path):
    ...
    if found:
        raise Exception(
            ...
            f'It must be built with -Dhelper=disabled: hesabdesk captures as root and never uses '
...
DRM_PACKAGE_NAME = 'hesabdesk-unattended-wayland'
...
def stage_libdrmtap_into_deb(so_path):
    ...
    # symlink is needed: libdrmtap is resolved by ABSOLUTE path (/usr/lib/hesabdesk/libdrmtap.so.0) at
    # the in-process dlopen site (drmtap_dl.rs), so the deb does NOT drop /usr/lib/hesabdesk into the
    ...
    system2('mkdir -p tmpdeb/usr/lib/hesabdesk')
    ...
    system2(f'cp "{so_path}" tmpdeb/usr/lib/hesabdesk/')
    system2(f'ln -sf "{so_basename}" tmpdeb/usr/lib/hesabdesk/libdrmtap.so.0')


def _max_glibc_minor(path):
    # Read from .dynstr rather than via objdump so packaging needs no binutils; chunked because
    # libhesabdesk.so is ~45 MB.
    best = 0
...
def measured_glibc_floor():
    ...
    paths = [p for p in glob.glob('tmpdeb/usr/lib/hesabdesk/libdrmtap.so.0.*')
             + glob.glob('tmpdeb/usr/share/hesabdesk/lib/libhesabdesk.so')
             + glob.glob('tmpdeb/usr/share/hesabdesk/hesabdesk')
...
def retarget_control_to_drm_variant():
    ...
    out = []
    for line in lines:
        if line.startswith('Package: hesabdesk'):
            out.append(f'Package: {DRM_PACKAGE_NAME}\n')
            out.append('Conflicts: hesabdesk\nReplaces: hesabdesk\nProvides: hesabdesk\n')
            ...
            out.append(line)
    body = ''.join(out)
    # Fail loudly rather than silently shipping a package that says `hesabdesk`: a stock control file
...
def build_flutter_deb(version, features):
    ...
    system2('mkdir -p tmpdeb/usr/share/hesabdesk')
    system2('mkdir -p tmpdeb/usr/share/hesabdesk/files/systemd/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/256x256/apps/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/scalable/apps/')
    system2('mkdir -p tmpdeb/usr/share/applications/')
    system2('mkdir -p tmpdeb/usr/share/polkit-1/actions')
    system2('rm tmpdeb/usr/bin/hesabdesk || true')
    system2(
        f'cp -r {flutter_build_dir}/* tmpdeb/usr/share/hesabdesk/')
    system2(
        'cp ../res/hesabdesk.service tmpdeb/usr/share/hesabdesk/files/systemd/')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/hesabdesk.png')
    system2(
        'cp ../res/scalable.svg tmpdeb/usr/share/icons/hicolor/scalable/apps/hesabdesk.svg')
    system2(
        'cp ../res/hesabdesk.desktop tmpdeb/usr/share/applications/hesabdesk.desktop')
    system2(
        'cp ../res/hesabdesk-link.desktop tmpdeb/usr/share/applications/hesabdesk-link.desktop')
    system2(
        "echo \"#!/bin/sh\" >> tmpdeb/usr/share/hesabdesk/files/polkit && chmod a+x tmpdeb/usr/share/hesabdesk/files/polkit")
    ...
    system2('dpkg-deb -b tmpdeb hesabdesk.deb;')

    system2('/bin/rm -rf tmpdeb/')
    system2('/bin/rm -rf ../res/DEBIAN/control')
    os.rename('hesabdesk.deb', '../hesabdesk-%s.deb' % version)
    if ships_so:
        # Named apart from the stock package so installing the consent-free variant is a deliberate act.
        os.rename('../hesabdesk-%s.deb' % version, f'../{DRM_PACKAGE_NAME}-{version}.deb')
    os.chdir("..")


DRMTAP_DLOPEN_MARKER = b'/usr/lib/hesabdesk/libdrmtap.so.0'
...
def _carries_drmtap_marker(path, marker=DRMTAP_DLOPEN_MARKER):
    ...
    # libhesabdesk.so is ~45 MB and there is no reason to hold it all in memory, and the `with`
...
def assert_staged_binary_is_drm():
    ...
    """
    binaries = [p for p in glob.glob('tmpdeb/usr/share/hesabdesk/lib/libhesabdesk.so')
                + glob.glob('tmpdeb/usr/share/hesabdesk/hesabdesk') if os.path.isfile(p)]
...
def build_deb_from_folder(version, binary_folder, want_drm=False):
    ...
    system2('rm tmpdeb/usr/bin/hesabdesk || true')
    system2(
        f'cp -r ../{binary_folder}/* tmpdeb/usr/share/hesabdesk/')
    system2(
        'cp ../res/hesabdesk.service tmpdeb/usr/share/hesabdesk/files/systemd/')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/hesabdesk.png')
    ...
    bundled_glob = glob.glob('tmpdeb/usr/share/hesabdesk/libdrmtap.so.0.*')
    ...
    if want_drm:
        ...
        if bundle_carries_so:
            ...
            system2(f'rm -f "{so}"')
            system2('rm -f tmpdeb/usr/share/hesabdesk/libdrmtap.so tmpdeb/usr/share/hesabdesk/libdrmtap.so.0')
        else:
...
def build_flutter_dmg(version, features):
    ...
    # copy dylib
    system2(
        "cp target/release/liblibhesabdesk.dylib target/release/libhesabdesk.dylib")
    os.chdir('flutter')
    ...
    system2('cp -rf ../target/release/service ./build/macos/Build/Products/Release/HesabDesk.app/Contents/MacOS/')
    '''
    system2(
        "create-dmg --volname \"HesabDesk Installer\" --window-pos 200 120 --window-size 800 400 --icon-size 100 --app-drop-link 600 185 --icon HesabDesk.app 200 190 --hide-extension HesabDesk.app hesabdesk.dmg ./build/macos/Build/Products/Release/HesabDesk.app")
    os.rename("hesabdesk.dmg", f"../hesabdesk-{version}.dmg")
    '''
    os.chdir("..")
...
def build_flutter_arch_manjaro(version, features):
    ...
    os.chdir('flutter')
    system2('flutter build linux --release')
    system2(f'strip {flutter_build_dir}/lib/libhesabdesk.so')
    os.chdir('../res')
...
def build_flutter_windows(version, features, skip_portable_pack):
    if not skip_cargo:
        system2(f'cargo build --locked --features {features} --lib --release')
        if not os.path.exists("target/release/libhesabdesk.dll"):
    ...
    system2(
        f'python3 ./generate.py -f ../../{flutter_build_dir_2} -o . -e ../../{flutter_build_dir_2}/hesabdesk.exe')
    os.chdir('../..')
    if os.path.exists('./hesabdesk_portable.exe'):
        os.replace('./target/release/hesabdesk-portable-packer.exe',
                   './hesabdesk_portable.exe')
    else:
        os.rename('./target/release/hesabdesk-portable-packer.exe',
                  './hesabdesk_portable.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/hesabdesk_portable.exe')
    os.rename('./hesabdesk_portable.exe', f'./hesabdesk-{version}-install.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/hesabdesk-{version}-install.exe')
...
def main():
    ...
    if windows:
        ...
        # system2('upx.exe target/release/hesabdesk.exe')
        system2('mv target/release/hesabdesk.exe target/release/HesabDesk.exe')
        pa = os.environ.get('P')
        if pa:
            # https://certera.com/kb/tutorial-guide-for-safenet-authentication-client-for-code-signing/
            system2(
                f'signtool sign /a /v /p {pa} /debug /f .\\cert.pfx /t http://timestamp.digicert.com  '
                'target\\release\\hesabdesk.exe')
        else:
            print('Not signed')
        os.makedirs(res_dir, exist_ok=True)
        system2(
            f'cp -rf target/release/HesabDesk.exe {res_dir}')
        os.chdir('libs/portable')
        system2('pip3 install -r requirements.txt')
        system2(
            f'python3 ./generate.py -f ../../{res_dir} -o . -e ../../{res_dir}/hesabdesk-{version}-win7-install.exe')
        system2(f'mv ../../{res_dir}/hesabdesk-{version}-win7-install.exe ../..')
        ...
        if flutter:
            ...
            system2('cargo build --locked --release --features ' + features)
            system2('git checkout src/ui/common.tis')
            system2('strip target/release/hesabdesk')
            system2('ln -s res/pacman_install && ln -s res/PKGBUILD')
            system2('HBB=`pwd` makepkg -f')
        system2('mv hesabdesk-%s-0-x86_64.pkg.tar.zst hesabdesk-%s-manjaro-arch.pkg.tar.zst' % (
            version, version))
        # pacman -U ./hesabdesk.pkg.tar.zst
        ...
        system2(
            'mv $HOME/rpmbuild/RPMS/x86_64/hesabdesk-%s-0.x86_64.rpm ./hesabdesk-%s-fedora28-centos8.rpm' % (
                version, version))
        # yum localinstall hesabdesk.rpm
        ...
        system2(
            'mv $HOME/rpmbuild/RPMS/x86_64/hesabdesk-%s-0.x86_64.rpm ./hesabdesk-%s-suse.rpm' % (
                version, version))
        # yum localinstall hesabdesk.rpm
    else:
        if flutter:
            if osx:
                ...
                # system2(
                #     'mv target/release/bundle/deb/hesabdesk*.deb ./flutter/hesabdesk.deb')
                build_flutter_deb(version, features)
        else:
            system2('cargo --locked bundle --release --features ' + features)
            if osx:
                system2(
                    'strip target/release/bundle/osx/HesabDesk.app/Contents/MacOS/hesabdesk')
                system2(
                    'cp libsciter.dylib target/release/bundle/osx/HesabDesk.app/Contents/MacOS/')
    ...
    #rcodesign sign --p12-file ~/.p12/hesabdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/HesabDesk.app/Contents/MacOS/hesabdesk
    #rcodesign sign --p12-file ~/.p12/hesabdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/HesabDesk.app/Contents/MacOS/libsciter.dylib
    #rcodesign sign --p12-file ~/.p12/hesabdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/HesabDesk.app
    # goto "Keychain Access" -> "My Certificates" for below id which starts with "Developer ID Application:"
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/HesabDesk.app/Contents/MacOS/*
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/HesabDesk.app
    '''.format(pa))
                system2(
                    'create-dmg "HesabDesk %s.dmg" "target/release/bundle/osx/HesabDesk.app"' % version)
                os.rename('HesabDesk %s.dmg' %
                          version, 'hesabdesk-%s.dmg' % version)
                if pa:
                    system2('''
    ...
    #rcodesign sign --p12-file ~/.p12/hesabdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./hesabdesk-{1}.dmg
    codesign -s "Developer ID Application: {0}" --force --options runtime ./hesabdesk-{1}.dmg
    ...
    rcodesign notary-submit --api-key-path ../.p12/api-key.json  --staple hesabdesk-{1}.dmg
    # verify:  spctl -a -t exec -v /Applications/HesabDesk.app
    '''.format(pa, version))
                else:
                    print('Not signed')
            else:
                # build deb package
                system2(
                    'mv target/release/bundle/deb/hesabdesk*.deb ./hesabdesk.deb')
                system2('dpkg-deb -R hesabdesk.deb tmpdeb')
                system2('mkdir -p tmpdeb/usr/share/hesabdesk/files/systemd/')
                system2('mkdir -p tmpdeb/usr/share/icons/hicolor/256x256/apps/')
                system2('mkdir -p tmpdeb/usr/share/icons/hicolor/scalable/apps/')
                system2(
                ...
                system2('strip tmpdeb/usr/bin/hesabdesk')
                system2('mkdir -p tmpdeb/usr/share/hesabdesk')
                system2('mv tmpdeb/usr/bin/hesabdesk tmpdeb/usr/share/hesabdesk/')
                system2('cp libsciter-gtk.so tmpdeb/usr/share/hesabdesk/')
                md5_file_folder("tmpdeb/")
                system2('dpkg-deb -b tmpdeb hesabdesk.deb; /bin/rm -rf tmpdeb/')
                os.rename('hesabdesk.deb', 'hesabdesk-%s.deb' % version)

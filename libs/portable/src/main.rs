...
const APP_PREFIX: &str = "hesabdesk";
const APPNAME_RUNTIME_ENV_KEY: &str = "HESABDESK_APPNAME";
#[cfg(windows)]
const SET_FOREGROUND_WINDOW_ENV_KEY: &str = "SET_FOREGROUND_WINDOW";

// The extraction directory follows whatever executable the payload asks for, so a
// custom client gets its own directory instead of sharing HesabDesk's. Falls back to
...
mod win {
    ...
    pub(super) fn copy_runtime_broker(dir: &Path) {
        ...
        let _allow_err = Command::new("taskkill")
            .args(&["/F", "/IM", "RuntimeBroker_hesabdesk.exe"])

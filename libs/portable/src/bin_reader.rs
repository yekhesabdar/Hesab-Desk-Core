...
mod tests {
    ...
    fn parses_the_generate_py_layout() {
        let (files, exe) = parse(blob(
            &[("./hesabdesk.exe", b"app"), ("./custom.txt", b"cfg")],
            "./hesabdesk.exe",
        ))
        .unwrap();
        assert_eq!(exe, "./hesabdesk.exe");
    ...
    fn rejects_malformed_blobs() {
        ...
        assert!(parse(b"hesabdesk\x00\x00\x00\x40partial".as_slice()).is_none());
    }
    ...
    fn without_a_package_the_stock_payload_is_untouched() {
        let embedded = parse(blob(&[("./hesabdesk.exe", b"app")], "./hesabdesk.exe")).unwrap();
        let (files, exe) = merge(embedded, Default::default());
        assert_eq!(exe, "./hesabdesk.exe");
        assert!(entry(&files, "./hesabdesk.exe").is_some());
    }
    ...
    fn renames_the_stock_executable_to_the_package_name() {
        ...
        let embedded = parse(blob(
            &[("./hesabdesk.exe", b"app"), ("./sciter.dll", b"dll")],
            "./hesabdesk.exe",
        ))
        .unwrap();
        ...
        assert!(entry(&files, "./hesabdesk.exe").is_none());
    ...
    fn package_entries_win_over_the_generic_payload() {
        ...
        assert_eq!(entry(&files, "./libhesabdesk.dll").unwrap().raw, b"core");
    }
    ...
    fn package_paths_are_recorded_for_the_dropped_file_sweep() {
        ...
        let embedded = parse(blob(&[("./libhesabdesk.dll", b"core")], "./hesabdesk.exe")).unwrap();

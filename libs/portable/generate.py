...
def write_blob(md5_table: dict, output_path: str, exe: str):
    with open(output_path, "wb") as f:
        f.write("hesabdesk".encode(encoding=encoding))
...
# Linux: python3 generate.py -f ../hesabdesk-portable-packer/test -o . -e ./test/main.py
# Windows: python3 .\generate.py -f ..\hesabdesk\flutter\build\windows\runner\Debug\ -o . -e ..\hesabdesk\flutter\build\windows\runner\Debug\hesabdesk.exe
...
if __name__ == '__main__':
    ...
    parser.add_option("-e", "--executable", dest="executable",
                      help="specify startup file in --folder, default is hesabdesk.exe")
    ...
    folder = options.folder or './hesabdesk'
    output_folder = os.path.abspath(options.output_folder or './')

    if not options.executable:
        options.executable = 'hesabdesk.exe'
    ...
    # `python generate.py -f hesabdesk -e hesabdesk.exe` or `python generate.py -f hesabdesk`

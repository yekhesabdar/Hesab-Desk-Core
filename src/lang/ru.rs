...
pub static ref T: std::collections::HashMap<&'static str, &'static str> =
    [
        ...
        ("connecting_status", "Подключение к сети HesabDesk..."),
        ...
        ("install_tip", "В некоторых случаях из-за UAC HesabDesk может работать неправильно на удалённом узле. Чтобы избежать возможных проблем с UAC, нажмите кнопку ниже для установки HesabDesk в системе."),
        ("Click to upgrade", "Нажмите, чтобы обновить"),
        ("Configure", "Настроить"),
        ("config_acc", "Чтобы удалённо управлять своим рабочим столом, вы должны предоставить HesabDesk права \"доступа\""),
        ("config_screen", "Для удалённого доступа к рабочему столу вы должны предоставить HesabDesk права \"снимок экрана\""),
        ...
        ("android_input_permission_tip1", "Чтобы удалённое устройство могло управлять вашим Android-устройством с помощью мыши или нажатий, необходимо разрешить HesabDesk использовать службу \"Специальные возможности\"."),
        ("android_input_permission_tip2", "Перейдите на соответствующую страницу системных настроек, найдите и войдите в \"Установленные службы\", включите службу \"HesabDesk Input\"."),
        ...
        ("Turned off", "Отключён"),
        ("Language", "Язык"),
        ("Keep HesabDesk background service", "Держать в фоне службу HesabDesk"),
        ...
        ("Show HesabDesk", "Показать HesabDesk"),
        ...
        ("config_input", "Чтобы управлять удалённым рабочим столом с помощью клавиатуры, необходимо предоставить HesabDesk разрешения \"Мониторинг ввода\"."),
        ("config_microphone", "Чтобы разговаривать с удалённой стороной, необходимо предоставить HesabDesk разрешение \"Запись аудио\"."),
        ...
        ("still_click_uac_tip", "По-прежнему требуется, чтобы удалённый пользователь нажал \"OK\" в окне UAC при запуске HesabDesk."),
        ...
        ("verify_hesabdesk_password_tip", "Подтвердить пароль HesabDesk"),
        ...
        ("scam_text1", "Если вы разговариваете по телефону с кем-то, кого вы НЕ ЗНАЕТЕ и НЕ ДОВЕРЯЕТЕ, и он просит вас использовать HesabDesk и запустить его службу, не продолжайте и немедленно прервите разговор."),
        ...
        ("upgrade_hesabdesk_server_pro_to_{}_tip", "Обновите HesabDesk Server Pro до версии {} или новее!"),
        ...
        ("selinux_tip", "На вашем устройстве включён SELinux, что может помешать правильной работе HesabDesk на управляемой стороне."),
        ("Change view", "Вид"),
        ...
        ("powered_by_me", "Основано на HesabDesk"),
        ...
        ("allow-only-conn-window-open-tip", "Разрешать подключение только при открытом окне HesabDesk"),
        ...
        ("floating_window_tip", "Помогает поддерживать фоновую службу HesabDesk"),
        ...
        ("upgrade_remote_hesabdesk_client_to_{}_tip", "Обновите клиент HesabDesk до версии {} или новее на удалённой стороне!"),
        ...
        ("allow-insecure-tls-fallback-tip", "По умолчанию HesabDesk проверяет сертификат сервера на наличие протоколов, использующих TLS.\nЕсли эта функция включена, HesabDesk пропустит данный этап и продолжит работу в случае неудачной проверки."),
        ("Disable UDP", "Отключить UDP"),
        ("disable-udp-tip", "Определяет, следует ли использовать только TCP.\nЕсли включено, HesabDesk не будет использовать UDP 21116, вместо него будет использоваться TCP 21116."),
        ("server-oss-not-support-tip", "ПРИМЕЧАНИЕ: в OSS-сервере HesabDesk эта функция отсутствует."),
        ...
        ("HesabDesk cannot reach the desktop session on the remote device, check that a desktop session is running and that HesabDesk can use it", "HesabDesk не может получить доступ к сеансу рабочего стола на удалённом устройстве, проверьте, что сеанс запущен и доступен HesabDesk"),
        ...
        ("HesabDesk could not obtain a usable screen from the XDG Desktop Portal, the PipeWire library may be too old", "HesabDesk не смог получить пригодный экран от XDG Desktop Portal, библиотека PipeWire может быть слишком старой"),
        ("HesabDesk could not load a GStreamer component needed for screen capture ({})", "HesabDesk не удалось загрузить компонент GStreamer, необходимый для захвата экрана ({})"),

...
pub fn portable_service_logon_helper_paths() -> Option<(PathBuf, PathBuf)> {
    ...
    let dir = user_dir
        ...
        .join("AppData")
        .join("Local")
        .join("hesabdesk-sciter");
    let dst = dir.join("hesabdesk.exe");
    Some((dir, dst))
}
...
pub fn install_me(options: &str, path: String, silent: bool, debug: bool) -> ResultType<()> {
    ...
    let tmp_path = "%HESABDESK_OUTPUT_DIR%".to_owned();
...
pub fn get_custom_client_staging_dir() -> PathBuf {
    get_public_base_dir()
        .join("HesabDesk")
        .join("HesabDeskCustomClientStaging")
}
...
/// - Deleting directories under a public location (e.g., C:\\ProgramData\\HesabDesk) is
...
pub fn bootstrap() -> bool {
    ...
    #[cfg(not(debug_assertions))]
    {
        // This function will cause `'sciter.dll' was not found neither in PATH nor near the current executable.` when debugging HesabDesk.
...
mod cert {
    ...
    extern "C" {
        fn DeleteHesabDeskTestCertsW();
    }
    pub fn uninstall_cert() -> ResultType<()> {
        unsafe {
            DeleteHesabDeskTestCertsW();
        }
        Ok(())
    }
}
...
// caller's own context, so installing or toggling the service from a HesabDesk
...
taskkill /F /IM {app_name}.exe{filter}
{tray_shortcut_commands}
copy /Y \"%HESABDESK_OUTPUT_DIR%\\{app_name} Tray.lnk\" \"%PROGRAMDATA%\\Microsoft\\Windows\\Start Menu\\Programs\\Startup\\\"
{import_config}
{create_service}
    ",
        import_config = get_import_config(exe),
...
reg add {subkey} /f /v EstimatedSize /t REG_DWORD /d {size}
    ...
    // 1. There may be some other processes like `hesabdesk --connect` are running.
...
fn build_msi_uninstall_command(product_code: &str) -> String {
    format!(
        "set \"HESABDESK_MSI_EXIT_CODE=\"\n\
MsiExec.exe /X {product_code} /norestart REBOOT=ReallySuppress\n\
set \"HESABDESK_MSI_EXIT_CODE=%ERRORLEVEL%\"\n\
if \"%HESABDESK_MSI_EXIT_CODE%\"==\"{MSI_EXIT_SUCCESS_REBOOT_REQUIRED}\" echo MSI uninstall succeeded with a reboot recommendation; continuing without reboot.\n\
if \"%HESABDESK_MSI_EXIT_CODE%\"==\"{MSI_EXIT_SUCCESS_REBOOT_INITIATED}\" echo MSI uninstall succeeded with a reboot request; continuing without forcing reboot.\n\
if not \"%HESABDESK_MSI_EXIT_CODE%\"==\"0\" if not \"%HESABDESK_MSI_EXIT_CODE%\"==\"{MSI_EXIT_SUCCESS_REBOOT_REQUIRED}\" if not \"%HESABDESK_MSI_EXIT_CODE%\"==\"{MSI_EXIT_SUCCESS_REBOOT_INITIATED}\" exit /b %HESABDESK_MSI_EXIT_CODE%\n\
ver > nul"
    )
}
    ...
    for entry in entries {
        if let Ok(entry) = entry {
            ...
            if let Some(file_name) = path.file_name().and_then(|n| n.to_str()) {
                // Match files like hesabdesk-*.msi or hesabdesk-*.exe
                if file_name.starts_with("hesabdesk-")
...
pub fn try_kill_hesabdesk_main_window_process() -> ResultType<()> {
    // Kill hesabdesk.exe without extra arg, should only be called by --server
    ...
    bail!("failed to find hesabdesk main window process");
}
...
pub fn send_raw_data_to_printer(printer_name: Option<String>, data: Vec<u8>) -> ResultType<()> {
    ...
    unsafe {
        ...
        if res != 0 {
            bail!("Failed to send data to the printer, see logs in C:\\Windows\\temp\\test_hesabdesk.log for more details.");
        } else {
...
fn get_pids_with_args_from_wmic_output<S2: AsRef<str>>(
    ...
    // ProcessId=34668
    //
    // CommandLine="C:\Program Files\HesabDesk\HesabDesk.exe" --tray
    // ProcessId=13728
    //
    // CommandLine="C:\Program Files\HesabDesk\HesabDesk.exe"
    // ProcessId=10136
    let mut pids = Vec::new();
...
mod tests {
    ...
    fn install_app_names_enforce_ascii_command_safety() {
        assert!(validate_install_app_name("HesabDesk-Admin1").is_ok());
        for app_name in ["", "HesabDesk_Admin", "HesabDesk&whoami", "HesabDesk应用"] {
            assert!(

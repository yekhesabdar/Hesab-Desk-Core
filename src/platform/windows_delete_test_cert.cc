...
//*************************************************************
//
//  DeleteHesabDeskTestCertsW_SingleHive()
//
//  Purpose:    Deletes HesabDesk Test certificates and wrong key stores
//
...
BOOL DeleteHesabDeskTestCertsW_SingleHive(HKEY RootKey, LPWSTR Prefix = NULL) {
...
extern "C" void DeleteHesabDeskTestCertsW() {
	// Current user
	std::wcout << "*** Current User" << std::endl;
	DeleteHesabDeskTestCertsW_SingleHive(HKEY_CURRENT_USER);

	// Local machine (requires admin rights)
	std::wcout << "*** Local Machine" << std::endl;
	DeleteHesabDeskTestCertsW_SingleHive(HKEY_LOCAL_MACHINE);
	...
	for (DWORD Index = 0; ; Index++) {
		...
		DeleteHesabDeskTestCertsW_SingleHive(HKEY_USERS, SubKeyName);
	}

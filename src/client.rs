...
pub const SCRAP_X11_REF_URL: &str = "https://hesabdesk.com/docs/en/manual/linux/#x11-required";
...
impl Client {
                    ...
                    Some(rendezvous_message::Union::RelayResponse(rr)) => {
                        ...
                        let race_result = if let Some(webrtc) = webrtc_for_connect {
                            ...
                            if interface.is_policy_relay() {
                                ...
                                // over the HesabDesk relay. Take the first successful relay instead
    ...
    async fn connect(
        ...
        // Keep a WebRTC win instead of replacing it with the HesabDesk relay: under relay-by-
    ...
    fn check_clipboard(&mut self) {
        if CLIPBOARD_STATE.lock().unwrap().running {
            ...
            if let Some(urls) = check_clipboard_files(&mut self.ctx, ClipboardSide::Client, false) {
                if !urls.is_empty() {
                    #[cfg(target_os = "macos")]
                    if crate::clipboard::is_file_url_set_by_hesabdesk(&urls) {
                        return;
                    }
...
lazy_static::lazy_static! {
    static ref LOGIN_ERROR_MAP: Arc<HashMap<&'static str, LoginErrorMsgBox>> = {
        let map = HashMap::from([(LOGIN_SCREEN_WAYLAND, LoginErrorMsgBox{
            ...
            link: "https://hesabdesk.com/docs/en/manual/linux/#login-screen",
            try_again: true,
...
async fn hc_connection_(
    ...
    let mut timer = crate::hesabdesk_interval(interval(crate::TIMER_OUT));

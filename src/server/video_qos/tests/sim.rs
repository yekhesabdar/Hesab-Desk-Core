...
fn write_traces(results: &[(Summary, Vec<Report>)]) {
    use std::fmt::Write;
    if let Ok(path) = std::env::var("HESABDESK_QOS_SIM_CSV") {
...
fn sim_scenarios() {
    ...
    if std::env::var("HESABDESK_QOS_SIM_VERBOSE").is_ok() {
        for (_, reports) in &results {
            for r in reports {
                println!(
...
/// Replays the log named by `HESABDESK_QOS_TRACE` and prints the result.
#[test]
fn replay_recorded_trace() {
    let Ok(path) = std::env::var("HESABDESK_QOS_TRACE") else {
        return;
    };
...
fn replay_recorded_trace_is_independent_of_connection_id() {
    let replay = |id: i32| {
        ...
        let path = std::env::temp_dir().join(format!(
            "hesabdesk-qos-replay-{}-{nonce}-{id}.log",
            std::process::id()
        ));

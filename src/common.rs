...
pub const PORTABLE_APPNAME_RUNTIME_ENV_KEY: &str = "HESABDESK_APPNAME";
...
// Because the url is always `https://api.hesabdesk.com/version/latest`.
#[tokio::main(flavor = "current_thread")]
pub async fn do_check_software_update() -> hbb_common::ResultType<()> {
    let (request, url) =
        hbb_common::version_check_request(hbb_common::VER_TYPE_HESABDESK_CLIENT.to_string());
...
#[inline]
pub fn is_hesabdesk() -> bool {
    hbb_common::config::APP_NAME.read().unwrap().eq("HesabDesk")
}
...
fn get_api_server_(api: String, custom: String) -> String {
    ...
    "https://admin.hesabdesk.com".to_owned()
}
...
pub fn is_public(url: &str) -> bool {
    ...
    host == "hesabdesk.com" || host.ends_with(".hesabdesk.com")
}
...
pub fn check_process(arg: &str, mut same_uid: bool) -> bool {
    ...
    for (_, p) in sys.processes().iter() {
        ...
        // on mac, p.cmd() get "/Applications/HesabDesk.app/Contents/MacOS/HesabDesk", "XPC_SERVICE_NAME=com.carriez.HesabDesk_server"
...
pub type HesabDeskInterval = ThrottledInterval;

#[inline]
pub fn hesabdesk_interval(i: Interval) -> ThrottledInterval {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_HesabDesk_interval() {
        ...
        for maker in base_intervals.into_iter() {
            ...
            let mut timer = hesabdesk_interval(maker());
    ...
    #[allow(non_snake_case)]
    #[tokio::test]
    async fn test_HesabDesk_interval_sleep() {
        let base_intervals = [interval_maker, interval_at_maker];
    ...
    #[test]
    fn test_is_public() {
        // Test URLs containing "hesabdesk.com/"
        assert!(is_public("https://hesabdesk.com/"));
        assert!(is_public("https://www.hesabdesk.com/"));
        assert!(is_public("https://api.hesabdesk.com/v1"));
        assert!(is_public("https://API.HESABDESK.COM/v1"));
        assert!(is_public("https://hesabdesk.com/path"));

        // Test URLs ending with "hesabdesk.com"
        assert!(is_public("hesabdesk.com"));
        assert!(is_public("https://hesabdesk.com"));
        assert!(is_public("https://HesabDesk.com"));
        assert!(is_public("http://www.hesabdesk.com"));
        assert!(is_public("https://api.hesabdesk.com"));
        ...
        assert!(!is_public("https://hesabdesk.computer.com"));
        assert!(!is_public("hesabdesk.comhello.com"));
    }

    #[test]
    fn test_is_public_matches_hesabdesk_root_domain() {
        assert!(is_public("hesabdesk.com/"));
        assert!(is_public("hesabdesk.com:21117"));
        assert!(is_public("api.hesabdesk.com:21117"));
        assert!(!is_public("hello-hesabdesk.com"));
        assert!(!is_public("api.hesabdesk.com.evil.test"));
        assert!(!is_public("https://hesabdesk.com@evil.test"));
    }
    ...
    fn test_should_use_tcp_proxy_for_api_url() {
        ...
        assert!(!should_use_tcp_proxy_for_api_url(
            "https://admin.hesabdesk.com/api/login",
            "https://admin.hesabdesk.com"
        ));
        assert!(!should_use_tcp_proxy_for_api_url(
            "https://admin.example.com/api/login",
            "not a url"

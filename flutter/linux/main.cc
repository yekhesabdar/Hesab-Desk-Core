...
typedef bool (*HesabDeskCoreMain)();
...
bool flutter_hesabdesk_core_main() {
   ...
   auto core_main = (HesabDeskCoreMain) dlsym(libhesabdesk,"hesabdesk_core_main");
   char* error;
   if ((error = dlerror()) != nullptr) {
       fprintf(stderr, "Program entry \"hesabdesk_core_main\" is not found: %s\n", error);
       return false;
   }
   return core_main();
}
...
int main(int argc, char** argv) {
  if (!flutter_hesabdesk_core_main()) {
      return 0;
  }

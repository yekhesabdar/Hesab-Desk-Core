...
typedef char** (*FUNC_HESABDESK_CORE_MAIN)(int*);
typedef void (*FUNC_HESABDESK_FREE_ARGS)( char**, int);
typedef int (*FUNC_HESABDESK_GET_APP_NAME)(wchar_t*, int);
typedef int (*FUNC_HESABDESK_IS_DISABLE_INSTALLATION)();
...
{
  HINSTANCE hInstance = LoadLibraryA("libhesabdesk.dll");
  if (!hInstance)
  {
    std::cout << "Failed to load libhesabdesk.dll." << std::endl;
    return EXIT_FAILURE;
  }
  FUNC_HESABDESK_CORE_MAIN hesabdesk_core_main =
      (FUNC_HESABDESK_CORE_MAIN)GetProcAddress(hInstance, "hesabdesk_core_main_args");
  if (!hesabdesk_core_main)
  {
    std::cout << "Failed to get hesabdesk_core_main." << std::endl;
    return EXIT_FAILURE;
  }
  FUNC_HESABDESK_FREE_ARGS free_c_args =
      (FUNC_HESABDESK_FREE_ARGS)GetProcAddress(hInstance, "free_c_args");
  if (!free_c_args)
  {
  ...
  int args_len = 0;
  char** c_args = hesabdesk_core_main(&args_len);
  if (!c_args)
  {
    std::string args_str = "";
    ...
    // std::cout << "HesabDesk [" << args_str << "], core returns false, exiting without launching Flutter app." << std::endl;
    return EXIT_SUCCESS;
  }
  std::vector<std::string> rust_args(c_args, c_args + args_len);
  free_c_args(c_args, args_len);
  FUNC_HESABDESK_IS_DISABLE_INSTALLATION hesabdesk_is_disable_installation =
      (FUNC_HESABDESK_IS_DISABLE_INSTALLATION)GetProcAddress(hInstance, "hesabdesk_is_disable_installation");
  bool is_disable_installation =
      hesabdesk_is_disable_installation && hesabdesk_is_disable_installation() != 0;
  ...
  std::wstring app_name = L"HesabDesk";
  FUNC_HESABDESK_GET_APP_NAME get_hesabdesk_app_name = (FUNC_HESABDESK_GET_APP_NAME)GetProcAddress(hInstance, "get_hesabdesk_app_name");
  if (get_hesabdesk_app_name) {
    wchar_t app_name_buffer[512] = {0};
    if (get_hesabdesk_app_name(app_name_buffer, 512) == 0) {

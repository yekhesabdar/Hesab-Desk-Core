...
/// The Linux bundle keeps the core library at lib/libhesabdesk.so next to the
...
DynamicLibrary _openLinuxCoreLib() {
  final bundled =
      '${File(Platform.resolvedExecutable).parent.path}/lib/libhesabdesk.so';
  try {
  ...
  return DynamicLibrary.open('libhesabdesk.so');
}
...
class PlatformFFI {
  ...
  HesabdeskImpl get ffiBind => _ffiBind;
  ...
  Future<void> init(String appType) async {
    ...
    final dylib = isAndroid
        ? DynamicLibrary.open('libhesabdesk.so')
        : isLinux
            ? _openLinuxCoreLib()
            : isWindows
                ? DynamicLibrary.open('libhesabdesk.dll')
                :
                ...
                //
                // isMacOS? DynamicLibrary.open("liblibhesabdesk.dylib") :
  ...
  void _startListenEvent(HesabdeskImpl hesabdeskImpl) {
    final appType =
        _appType == kAppTypeDesktopRemote ? '$_appType,$kWindowId' : _appType;
    var sink = hesabdeskImpl.startGlobalEventStream(appType: appType);
    sink.listen((message) {
      () async {
        try {

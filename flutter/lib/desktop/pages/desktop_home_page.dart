...
class _DesktopHomePageState extends State<DesktopHomePage>
                        ...
                        child: GestureDetector(
                          ...
                          child: TextFormField(
                            ...
                            decoration: InputDecoration(
                              border: InputBorder.none,
                              contentPadding:
                                  EdgeInsets.only(top: 14, bottom: 10, left: 7),
                            ),
  ...
  Widget buildHelpCards(String updateUrl) {
    if (!bind.isCustomClient() &&
        ...
        !isCardClosed &&
        bind.mainUriPrefixSync().contains('hesabdesk')) {
      final isToUpdate = (isWindows || isMacOS) && bind.mainIsInstalled();
      String btnText = isToUpdate ? 'Update' : 'Download';
      GestureTapCallback onPressed = () async {
        final Uri url = Uri.parse('https://hesabdesk.com/download');
        await launchUrl(url);
      };
      if (isToUpdate) {
        onPressed = () {
    ...
    if (isWindows && !bind.isDisableInstallation()) {
      ...
      if (bind.isSelinuxEnforcing()) {
        ...
        if (bind.mainGetLocalOption(key: keyShowSelinuxHelpTip) != 'N') {
          LinuxCards.add(buildInstallCard(
            ...
            help: 'Help',
            link:
                'https://hesabdesk.com/docs/en/client/linux/#permissions-issue',
            closeButton: true,
      ...
      if (bind.mainCurrentIsWayland()) {
        LinuxCards.add(buildInstallCard(
            ...
            marginTop: LinuxCards.isEmpty ? 20.0 : 5.0,
            help: 'Help',
            link: 'https://hesabdesk.com/docs/en/client/linux/#x11-required'));
      } else if (bind.mainIsLoginWayland()) {
        LinuxCards.add(buildInstallCard("Warning",
            "Login screen using Wayland is not supported", "", () async {},
            marginTop: LinuxCards.isEmpty ? 20.0 : 5.0,
            help: 'Help',
            link: 'https://hesabdesk.com/docs/en/client/linux/#login-screen'));
      }
  ...
  void initState() {
    ...
    hesabDeskWinManager.registerActiveWindowListener(onActiveWindowChanged);
    ...
    hesabDeskWinManager.setMethodHandler((call, fromWindowId) async {
      ...
      if (call.method == kWindowMainWindowOnTop) {
        ...
        await hesabDeskWinManager.registerActiveWindow(call.arguments["id"]);
      } else if (call.method == kWindowEventHide) {
        await hesabDeskWinManager.unregisterActiveWindow(call.arguments['id']);
        ...
        if (windowId != null && windowType != null) {
          await hesabDeskWinManager.moveTabToNewWindow(
        ...
        await hesabDeskWinManager.openMonitorSession(
            windowId, peerId, display, displayCount, screenRect, windowType);
      } else if (call.method == kWindowEventRemoteWindowCoords) {
        final windowId = int.tryParse(call.arguments);
        if (windowId != null) {
          return jsonEncode(
              await hesabDeskWinManager.getOtherRemoteWindowCoords(windowId));
        }
      }
    });
